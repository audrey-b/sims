...
The seed should be set using `set.seed()` not the `seed` argument of `furrr::future_options()` which is passed as an argument to `sims_simulate()` and `sims_add()` to control the environment the expressions are called in.
