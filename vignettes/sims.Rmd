...
Parallelization is achieved using the [furrr](https://github.com/DavisVaughan/furrr) package (which necessitates the loading of the `purrr` package)
...
The seed should be set using `set.seed()` not the `seed` argument of `furrr::future_options()`. The latter is however passed as an argument to `sims_simulate()` and `sims_add()` to control the environment the expressions are called in.
